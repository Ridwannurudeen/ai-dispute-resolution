...
const DISPUTE_ABI = [
    ...
    "function deliverAIVerdict(bytes32,uint8,uint8,string)",
    ...
    "event AIVerdictRequested(uint256 indexed disputeId, bytes32 indexed requestId)"
];
...
export default function DisputeResolutionDApp() {
    ...
    const requestVerdict = async () => {
        ...
        try {
            ...
            await tx.wait();
            setSuccess('Verdict requested! Waiting for AI analysis...');
            await loadDisputes();
    ...
    const simulateVerdict = async (resolution) => {
        if (!contract || !selectedDispute) return;
        
        try {
            setLoading(true);
            setError('');
            setSuccess('Finding verdict request...');
            
            // Get the request ID from events
            const filter = contract.filters.AIVerdictRequested(selectedDispute.id);
            const events = await contract.queryFilter(filter);
            
            if (events.length === 0) {
                setError('No verdict request found. Request AI verdict first.');
                setLoading(false);
                return;
            }
            
            const requestId = events[events.length - 1].args[1];
            console.log("Request ID:", requestId);
            
            const confidence = 75 + Math.floor(Math.random() * 20); // 75-95% confidence
            
            const tx = await contract.deliverAIVerdict(
                requestId, 
                resolution, 
                confidence, 
                "QmSimulatedAIVerdict" + Date.now()
            );
            setSuccess('Delivering AI verdict...');
            await tx.wait();
            setSuccess(`Verdict delivered! ${RESOLUTION_NAMES[resolution]} (${confidence}% confidence)`);
            await loadDisputes();
            
            // Update selected dispute
            const updatedDispute = await contract.getDispute(selectedDispute.id);
            setSelectedDispute(prev => ({
                ...prev,
                status: Number(updatedDispute[7]),
                resolution: Number(updatedDispute[8])
            }));
            
            setTimeout(() => setSuccess(''), 5000);
        } catch (err) {
            console.error("Simulate verdict error:", err);
            setError(err.reason || err.message);
        } finally {
            setLoading(false);
        }
    ...
    const finalizeDispute = async () => {
        ...
        try {
            ...
            await tx.wait();
            setSuccess('Dispute finalized! Funds distributed.');
            await loadDisputes();
        ...
        <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100">
                            ...
                            {selectedDispute ? (
                                <div className="bg-white rounded-xl shadow-sm p-6">
                                    ...
                                    <div className="space-y-4">
                                        ...
                                        {/* SIMULATE VERDICT - when Awaiting AI (status 2) */}
                                        {selectedDispute.status === 2 && (
                                            <div className="p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
                                                <h4 className="font-medium text-yellow-800 mb-3">
                                                    🧪 Simulate AI Verdict (Testing Mode)
                                                </h4>
                                                <p className="text-sm text-yellow-700 mb-4">
                                                    GenLayer AI is not connected. Use these buttons to simulate a verdict for testing.
                                                </p>
                                                <div className="flex gap-2 flex-wrap">
                                                    <button
                                                        onClick={() => simulateVerdict(1)}
                                                        disabled={loading}
                                        ...
                                        {/* Finalize - when verdict delivered */}
                                        {(selectedDispute.status === 3 || selectedDispute.status === 4) && (
                                            <button
                                                ...
                                                className="w-full px-4 py-3 bg-green-600 text-white rounded-lg hover:bg-green-700 disabled:opacity-50"
                                            >
                                                ✅ Finalize Dispute & Distribute Funds
                                            </button>
                                        )}
            ...
            <footer className="bg-white border-t mt-12">
                <div className="max-w-7xl mx-auto px-4 py-6 text-center text-gray-500 text-sm">
                    ...
                    <span className="text-yellow-600 ml-2">🧪 Testnet Mode</span>
                </div>
            </footer>
        </div>
    );
